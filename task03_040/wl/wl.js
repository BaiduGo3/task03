...
function addTdClickEvent(ele,type,tag,handler){
    addEvent(ele,type,function(event){
        ...
        if(target && target.tagName == tag.toUpperCase())
            handler.call(target,calendarObj.year,calendarObj.month);
    });
}

function Calendar(beginYear,endYear,year,month,day){
...
Calendar.prototype.render = function(){
    ...
    var firstDay = new Date(formatDate(this.year,this.month+1,1));
    renderTd(firstDay,this.day);
    function renderTd(firstDay,day){
...
Calendar.prototype.getChoiceDate = function(year,month){
    ...
    month += 1;
    alert("当前选中日期为："+year+"-"+month+"-"+this.innerHTML);
}
...
var calendarObj = new Calendar(2014,2017,2016,5,7);
